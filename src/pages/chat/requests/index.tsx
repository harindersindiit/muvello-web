            ...
            <div className="bg-[#111] border-r border-gray-800 p-4 pb-0">
                <div className="flex items-center gap-2 mb-4">
                    <div className="flex items-center gap-2">
                        <div className="text-lg font-bold mb-0 cursor-pointer flex items-center gap-1" onClick={() => navigate("/user/chat")}>
                            <Icon icon="uil:arrow-left" style={{ width: "30px", height: "30px" }} /> Requests
                        </div>
                    </div>
                    <div className={`bg-lightdark rounded-full p-2 cursor-pointer hover:bg-white hover:text-black ms-auto`}>
                ...
                <ScrollArea className="h-[calc(100vh-230px)]">
                    <div className="text-white mt-3">
                        {users.map((user) => {
                            const isActive = selectedChat?.id === user.id && activeTab === "users";
                                ...
                                >
                                    ...
                                    <div className="flex-1 border-b border-gray-700 pb-3 mb-7">
                                        <div className="flex justify-between items-center">
                                            <h3
                                                className={`font-semibold mb-1 text-sm ${isActive ? "text-lime-400" : "text-white"
                                                    }`}
                                            >
                                                {user.name}
                                            </h3>
                                            <span className="text-xs text-grey">{user.time}</span>
                                        </div>
                                        <div className="flex justify-between items-center">
            ...
            <div className="flex flex-col h-full w-full">
                ...
                <div className="border-b border-gray-800 p-4 font-semibold text-lg flex items-center gap-2 " >
                    <Avatar className="w-12 h-12 cursor-pointer">
                        <AvatarImage src={selectedChat?.image || selectedChat?.img || IMAGES.user1} />
                    ...
                    <div className="relative ms-auto">
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild className="cursor-pointer focus:outline-none focus:ring-0 focus:ring-offset-0">
                                <Link
                                    to="#"
                            ...
                            >
                                ...
                                <DropdownMenuItem className="text-white hover:opacity-80 mb-2">
                                    <Link to="#" className="flex items-center gap-2">
                                        <img src={IMAGES.reportuser} alt="Delete Group" /> Report User
                                    </Link>
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                ...
                <ScrollArea className="h-[calc(100vh-250px)] px-4 sm:px-6 py-4 space-y-4">
                    {chatMessages.map((msg: Message, i: number) => (
                        <div key={i} className={`flex mb-3 ${msg.isUser ? "justify-end" : "justify-start"}`}>
                            <div className="flex items-end gap-3">

                                <div>
                                    <div className={`bg-lightdark rounded-[22px] p-3 px-4 max-w-[90%] sm:max-w-md ${msg.isUser ? "bg-lime-500 text-black rounded-br-none" : "text-white rounded-bl-none"}`}>
                                        <p className="text-sm mb-0">{msg.usermessage}</p>
                                    </div>
                                    <span className={`block text-xs mt-1 text-grey font-normal ${msg.isUser ? "text-right" : "text-left"}`}>{msg.time}</span>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="p-4 pt-0 flex items-center gap-3">
                    <CustomButton text="Delete" type="button" className="w-auto flex-1 bg-red text-white border-none" />
                    <CustomButton text="Accept" type="button" onClick={() => navigate("/user/chat")} className="w-auto flex-1" />
                </div>
            </div>
        </div>
    );
